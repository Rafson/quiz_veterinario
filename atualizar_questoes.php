...
echo "<h2>Atualizando questões do banco de dados...</h2>\n";

$conn = getConnection();

// Adicionar coluna explicacao se não existir
echo "<p>1. Verificando estrutura da tabela...</p>\n";
$result = $conn->query("SHOW COLUMNS FROM questoes LIKE 'explicacao'");
if ($result->num_rows == 0) {
    echo "<p>Adicionando coluna 'explicacao'...</p>\n";
    $conn->query("ALTER TABLE questoes ADD COLUMN explicacao TEXT AFTER pergunta");
    echo "<p style='color: green;'>✓ Coluna 'explicacao' adicionada!</p>\n";
} else {
    echo "<p style='color: blue;'>ℹ Coluna 'explicacao' já existe.</p>\n";
}

// Limpar todas as questões antigas
echo "<p>2. Limpando questões antigas...</p>\n";
$conn->query("DELETE FROM respostas");
$conn->query("DELETE FROM participantes");
$conn->query("DELETE FROM questoes");
echo "<p style='color: green;'>✓ Questões antigas removidas!</p>\n";

// Inserir as 25 novas questões
echo "<p>3. Inserindo 25 novas questões...</p>\n";
...
$stmt = $conn->prepare("INSERT INTO questoes (numero, pergunta, explicacao, opcao_a, opcao_b, opcao_c, opcao_d, resposta_correta) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");

$inseridas = 0;
foreach ($questoes as $q) {
    $stmt->bind_param("isssssss", $q[0], $q[1], $q[2], $q[3], $q[4], $q[5], $q[6], $q[7]);
    if ($stmt->execute()) {
        $inseridas++;
    }
}

echo "<p style='color: green;'>✓ {$inseridas} questões inseridas com sucesso!</p>\n";

// Verificar total de questões
$result = $conn->query("SELECT COUNT(*) as total FROM questoes");
$row = $result->fetch_assoc();
echo "<p><strong>Total de questões no banco: {$row['total']}</strong></p>\n";

// Listar algumas questões
echo "<h3>Primeiras 5 questões cadastradas:</h3>\n";
$result = $conn->query("SELECT numero, pergunta FROM questoes ORDER BY numero LIMIT 5");
echo "<ol>\n";
while ($row = $result->fetch_assoc()) {
    echo "<li><strong>Questão {$row['numero']}:</strong> {$row['pergunta']}</li>\n";
}
echo "</ol>\n";
...
echo "<p><a href='index.php'>← Voltar para o Quiz</a></p>\n";
?>
